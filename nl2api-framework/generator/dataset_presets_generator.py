...
def build_verifier_user_message(user_query: str, tool: dict, params) -> str:
    required = [p for p in tool["parameters"] if p["required"]]
    optional = [p for p in tool["parameters"] if not p["required"]]

    required_str = ", ".join(
        f"{p['name']} ({p['type']}): {p['description']}" for p in required
    )
    optional_str = ", ".join(
        f"{p['name']} ({p['type']}, default={p.get('default', 'none')}): {p['description']}"
        for p in optional
    ) or "none"

    return (
        f"You are a strict validator of tool calls.\n\n"

        f"TOOL NAME:\n{tool['name']}\n\n"

        f"REQUIRED PARAMETERS (must appear or be clearly inferable from the query):\n{required}\n\n"
        f"OPTIONAL PARAMETERS (may be omitted, have defaults):\n{optional}\n\n"

        f"USER QUERY:\n\"{user_query}\"\n\n"

        f"TOOL CALL PARAMETERS (JSON):\n{params}\n\n"

        f"Your task is to verify whether the tool call is CORRECT and COMPLETE.\n\n"

        f"For each parameter from the tool call, you must check if it is clearly mentioned and inferable from the query\n"

        f"Output format (STRICT JSON ONLY, no text):\n"
        f"If everything is correct:\n"
        f"{{\"valid\": true}}\n\n"

        f"If there are problems:\n"
        f"{{\n"
        f"  \"valid\": false,\n"
        f"  \"incorrect\": [\"param1\", ...]\n"
        f"}}\n\n"
...
def build_verifier_example(user_query: str, tool: dict, params, complete: bool, omitted: list[str]) -> dict:
    user_message = build_verifier_user_message(user_query, tool, params)

    if complete:
        assistant_message = json.dumps({"complete": True})
    else:
        assistant_message = json.dumps({"complete": False, "missing": omitted})
...
def run_dataset_generation(n_samples_completas: int = 50):
    os.makedirs("dataset_raw4", exist_ok=True)

    final_dataset          = []  # tool calling
    final_dataset_mlp      = []  # MLP (binario)
    final_dataset_verifier = []  # verifier (conversacional)
    ...
    for tool in TOOLS_SPEC:
        ...
        while samples_count < n_samples_completas:
            ...
            for params in params_list:
                ...
                # Dataset verifier — completo
                tool_results_verifier.append(
                    build_verifier_example(user_query, tool, params_with_defaults, complete=True, omitted=[])
                )
                ...
                # Dataset verifier — incompleto
                tool_results_verifier.append(
                    build_verifier_example(incomplete_query, tool, keep, complete=False, omitted=omit)
                )

                print(f"[{tool['name']}] incompleto generado (omitido: {omit})")

        # Guardar por tool
        with open(f"dataset_raw4/{tool['name']}.json", "w", encoding="utf-8") as f:
            json.dump(tool_results, f, indent=4, ensure_ascii=False)

        with open(f"dataset_raw4/{tool['name']}_mlp.json", "w", encoding="utf-8") as f:
            json.dump(tool_results_mlp, f, indent=4, ensure_ascii=False)

        with open(f"dataset_raw4/{tool['name']}_verifier.json", "w", encoding="utf-8") as f:
    ...
    with open("dataset_raw4/full_training_dataset.json", "w", encoding="utf-8") as f:
        json.dump(final_dataset, f, indent=4, ensure_ascii=False)

    with open("dataset_raw4/full_training_dataset_mlp.json", "w", encoding="utf-8") as f:
        json.dump(final_dataset_mlp, f, indent=4, ensure_ascii=False)

    # Verifier en JSONL para SFTTrainer
    with open("dataset_raw4/full_training_dataset_verifier.jsonl", "w", encoding="utf-8") as f:
    ...
    n_ver_completos   = sum(1 for e in final_dataset_verifier if json.loads(e["messages"][1]["content"]).get("complete"))
    n_ver_incompletos = sum(1 for e in final_dataset_verifier if not json.loads(e["messages"][1]["content"]).get("complete"))
    ...
    print(f"✅ Dataset verifier:     {len(final_dataset_verifier)} ejemplos ({n_ver_completos} completos, {n_ver_incompletos} incompletos)")


if __name__ == "__main__":
    run_dataset_generation(50)