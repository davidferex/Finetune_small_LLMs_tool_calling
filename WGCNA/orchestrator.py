...
class BioOrchestrator:
    ...
    async def run(self, research_question: str):
        ...
        # Load the data
        aux = await self._load_data()

        print(aux.content[0].text)
        ...
        while self.pipeline_active:
            ...
            if resultado["tool"]:
                ...
                if resultado["tool"] == "repeat_pipeline":
            ...
            if not self.pipeline_active:
                ...
                self.adapter.set_active_tools(["DONE"])

                output = await self.session.call_tool("expose_results_to_web", {})
                ...
                output = await self.session.call_tool("notebook_creation", {})
                content = output.content[0].text
                print(content)
    ...
    async def _load_data(self):
        try:
            output = await self.session.call_tool("load_data", {})
            return output
        except Exception as e:
            print(f"⚠️ Failed to load the data: {e}")
            return {}